...
def all(ctx):
	...
	ctx.invoke(load_states)
	ctx.invoke(load_regions)
	ctx.invoke(load_continents)
	ctx.invoke(load_territories)
	ctx.invoke(load_economic_blocks)
	ctx.invoke(load_municipalities)
	ctx.invoke(load_industries)
	ctx.invoke(load_hedu_course)
	ctx.invoke(load_establishments)
	ctx.invoke(load_inflections)
	ctx.invoke(load_metadata_command)
